...
gt extractfeat -join -retainids -matchdescstart -type CDS -seqfile $scaf -o ${name}.cds.fa $gff 2> /dev/null
if [[ "$?" -eq 1 ]]
then
	gt gff3 -sort -retainids $gff > ${name}.sorted.gff
	if [[ "$?" -eq 0 ]]
	then
		gt extractfeat -join -retainids -matchdescstart -type CDS -seqfile $scaf -force -o ${name}.cds.fa ${name}.sorted.gff
	else
		exit 1
	fi
fi

sed -i 's/(joined)$//' ${name}.cds.fa
